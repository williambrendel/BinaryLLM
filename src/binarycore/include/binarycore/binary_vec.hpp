...
class BinaryVec {
  ...
  constexpr explicit BinaryVec(std::array<uint64_t, Chunks> chunks) noexcept : data_(chunks) {}
  ...
  constexpr bool get(std::size_t i) const noexcept { return (data_[i / 64] >> (i % 64)) & 1ULL; }
  constexpr void set(std::size_t i) noexcept { data_[i / 64] |= (1ULL << (i % 64)); }
  constexpr void clear(std::size_t i) noexcept { data_[i / 64] &= ~(1ULL << (i % 64)); }
