...
bits::Caps classify_caps(std::string_view word) noexcept {
    ...
    if (first_upper && !any_lower) return bits::Caps::AllUpper; // single-letter caps like "A"
    if (any_upper && any_lower) {
        // First letter capital and rest lowercase → FirstUpper
        // Any other mixed pattern → Mixed
...
uint64_t encode_word(std::string_view word) noexcept {
    ...
    // Pass over the word once, accumulating: letter presence, digit presence,
    // first letter, doubled-letter pairs, length.
    ...
    for (char c : word) {
        if (is_ascii_letter(c)) {
            ...
            // Letter presence (bits 0..25)
            v |= (1ULL << (bits::LetterPresenceStart + letter_idx));

            // First letter (bits 26..30, 5-bit unsigned)
            if (!first_letter_set) {
                v |= (static_cast<uint64_t>(letter_idx) << bits::FirstLetterStart);
                first_letter_set = true;
            }

            // Doubled letters (consecutive same letter, lowercase compared)
            if (lc == prev_lower) {
            ...
            prev_lower = 0; // digit breaks letter-run, no double across it
        } else {
            // Other characters (shouldn't appear in tokens passed to encode_word,
            // but be defensive): ignore for encoding, reset doubling state.
            prev_lower = 0;
        }
    }

    // Capitalization (bits 31..32)
    v |= (static_cast<uint64_t>(classify_caps(word)) << bits::CapsStart);

    // Length bucket (bits 33..36)
    v |= (length_bucket(word.size()) << bits::LengthBucketStart);

    // Type flag bit 63 = 0 for words (already zero, no-op).
    return v;
}
...
uint64_t encode_symbol(bits::Symbol s) noexcept {
    ...
    v |= (1ULL << bits::Type); // mark as symbol
    return v;
}
